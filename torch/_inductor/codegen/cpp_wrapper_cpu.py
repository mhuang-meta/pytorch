...
class CppWrapperCpu(PythonWrapperCodegen):
    ...
    def _get_profiling_input_handles(
        self, inputs: list[ir.IRNode], args: list[str]
    ) -> list[str]:
        ...
        *,
        ...
        with debug_printer_manager:
            ...
            if enable_kernel_profile:
                ...
                has_profiling_inputs = (
                    input_handles or num_scalars > 0 or output_handle
                )
    ...
    def generate_c_shim_extern_kernel_alloc(
        ...
        num_kwargs = sum(
            1 for key in extern_kernel.ordered_kwargs_for_cpp_kernel
            if key != "out"
        )
        self.generate_c_shim_extern_kernel_call(
            ...
            args,
            device,
            input_handles=self._get_profiling_input_handles(
                extern_kernel.inputs, args
            ),
    ...
    def generate_c_shim_fallback_kernel(
        ...
        input_handles = [
            inp.get_name() for inp in fallback_kernel.inputs
        ]
        ...
        num_kwargs = sum(
            1 for key in fallback_kernel.ordered_kwargs_for_cpp_kernel
            if key != "out"
        )
        self.generate_c_shim_extern_kernel_call(
